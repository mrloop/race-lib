...
module.exports = class Event {
  ...
  load() {
    return fs.readFileAsync(path.join(__dirname, 'sample', 'event_with_entrants.html')).then( file => file.toString());
  }
  ...
  static getUpcomming() {
    if(process.env.test) {
      return fs.readFileAsync(path.join(__dirname, 'sample', 'events.html')).then( file => file.toString());
    } else {
